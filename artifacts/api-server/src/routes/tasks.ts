...
router.post("/tasks", requirePermission("tasks.create"), async (req, res): Promise<void> => {
  ...
  const [row] = await db.insert(tasksTable).values({ title: d.title, description: d.description, status: d.status ?? "todo", priority: d.priority ?? "medium", projectId: d.projectId, assigneeId: d.assigneeId, dueDate: d.dueDate }).returning();
...
router.get("/tasks/summary", requirePermission("tasks.read"), async (req, res): Promise<void> => {
  const ids = await projectIdsForTenant(tenantId(req)); const tasks = (await db.select().from(tasksTable)).filter((t) => ids.includes(t.projectId)); const today = new Date().toISOString().split("T")[0];
...
router.get("/tasks/:id", requirePermission("tasks.read"), async (req, res): Promise<void> => {
  const [row] = await db.select().from(tasksTable).innerJoin(projectsTable, eq(tasksTable.projectId, projectsTable.id)).where(and(eq(tasksTable.id, Number(req.params.id)), eq(projectsTable.organizationId, tenantId(req))));
...
router.patch("/tasks/:id", requirePermission("tasks.update"), async (req, res): Promise<void> => {
  ...
  const owned = await db.select({ id: tasksTable.id }).from(tasksTable).innerJoin(projectsTable, eq(tasksTable.projectId, projectsTable.id)).where(and(eq(tasksTable.id, Number(req.params.id)), eq(projectsTable.organizationId, tenantId(req))));
  if (!owned.length) { res.status(404).json({ error: "Not found" }); return; }
  const [task] = await db.update(tasksTable).set(updates).where(eq(tasksTable.id, Number(req.params.id))).returning();
  const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, task.projectId)); const [assignee] = task.assigneeId ? await db.select().from(usersTable).where(and(eq(usersTable.id, task.assigneeId), eq(usersTable.organizationId, tenantId(req)))) : [];
