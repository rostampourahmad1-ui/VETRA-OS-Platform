...
router.get("/contracts", requirePermission("contracts.read"), async (req, res): Promise<void> => {
  ...
  let rows = (await db.select().from(contractsTable)).filter((c) => projectIds.has(c.projectId));
...
router.post("/contracts", requirePermission("contracts.create"), async (req, res): Promise<void> => {
  ...
  const [row] = await db.insert(contractsTable).values({ name: d.name, contractor: d.contractor, value: d.value.toString(), status: d.status ?? "draft", type: d.type, projectId: d.projectId, startDate: d.startDate, endDate: d.endDate, signedDate: d.signedDate }).returning();
...
router.get("/contracts/:id", requirePermission("contracts.read"), async (req, res): Promise<void> => {
  const [row] = await db.select().from(contractsTable).where(eq(contractsTable.id, Number(req.params.id)));
...
router.patch("/contracts/:id", requirePermission("contracts.update"), async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [current] = await db.select().from(contractsTable).where(eq(contractsTable.id, Number(req.params.id)));
  ...
  const [row] = await db.update(contractsTable).set(updates).where(eq(contractsTable.id, current.id)).returning();
  const [project] = await db.select().from(projectsTable).where(and(eq(projectsTable.id, row.projectId), eq(projectsTable.organizationId, tenantId(req))));
  res.json(serialize(row, project?.name ?? null));
});
