  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-card p-4 rounded-lg border shadow-sm">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <Button variant="outline" className="gap-2">
            <Filter className="h-4 w-4" /> {t('contracts.filterStatus')}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {isLoading ? (
          <div className="col-span-full py-20 text-center font-mono text-muted-foreground">{t('contracts.loading')}</div>
        ) : (
            ...
            <Card key={contract.id} className="group hover:border-primary/50 transition-colors">
              <CardContent className="p-5 space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <Badge variant="outline" className={`font-mono text-[10px] ${getStatusColor(contract.status)}`}>
                    {t(statusKeys[contract.status] ?? 'contracts.status.draft')}
                  </Badge>
                </div>

                <div className="pt-4 border-t border-border flex justify-between items-end">
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-mono tracking-wider text-muted-foreground">{t('contracts.projectLabel')}</span>
                  ...
                  <div className="flex flex-col gap-1 text-end">
                    <span className="text-[10px] font-mono tracking-wider text-muted-foreground">{t('contracts.valueLabel')}</span>
