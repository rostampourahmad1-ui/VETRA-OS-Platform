...
export default function QualityManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent><form className="grid gap-3" onSubmit={createNcr}>
            <div className="grid gap-3 sm:grid-cols-2"><Input required type="number" min="1" aria-label="پروژهٔ فعال" value={project?.id ?? ''} readOnly /><Input required placeholder="NCR title" value={ncrForm.title} onChange={(e) => setNcrForm({ ...ncrForm, title: e.target.value })} /></div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle>Inspections <span className="text-sm font-normal text-muted-foreground">({visibleInspections.length})</span></CardTitle><select className="h-9 rounded-md border bg-background px-2 text-sm" value={inspectionFilter} onChange={(e) => setInspectionFilter(e.target.value)}><option value="all">All statuses</option>{inspectionStatuses.map((status) => <option key={status} value={status}>{status.replace('_', ' ')}</option>)}</select></div></CardHeader><CardContent>{loading ? <p className="text-sm text-muted-foreground">Loading…</p> : visibleInspections.length === 0 ? <p className="text-sm text-muted-foreground">No inspections match this filter.</p> : <div className="space-y-3">{visibleInspections.map((item) => <div key={item.id} className="rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-medium">{item.title}</h3><p className="text-sm text-muted-foreground">Project #{item.projectId} · {item.type} · {item.inspector}</p></div><div className="flex items-center gap-2"><Badge variant={statusTone(item.status)}>{item.status.replace('_', ' ')}</Badge>{nextInspectionStatus[item.status] && <Button variant="outline" size="sm" disabled={transitioning === item.id} onClick={() => void transition('inspection', item.id, nextInspectionStatus[item.status] ?? '')}>تغییر به {nextInspectionStatus[item.status]}</Button>}<Button variant="ghost" size="icon" aria-label="حذف بازرسی" onClick={() => void remove(`/inspections/${item.id}`)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div></div><p className="mt-2 text-sm">Inspection date: {formatJalali(item.date)}</p>{item.findings && <p className="mt-1 text-sm text-muted-foreground">{item.findings}</p>}</div>)}</div>}</CardContent></Card>

        <Card><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle>NCRs <span className="text-sm font-normal text-muted-foreground">({visibleNcrs.length})</span></CardTitle><select className="h-9 rounded-md border bg-background px-2 text-sm" value={ncrFilter} onChange={(e) => setNcrFilter(e.target.value)}><option value="all">All statuses</option>{ncrStatuses.map((status) => <option key={status} value={status}>{status.replace('_', ' ')}</option>)}</select></div></CardHeader><CardContent>{loading ? <p className="text-sm text-muted-foreground">Loading…</p> : visibleNcrs.length === 0 ? <p className="text-sm text-muted-foreground">No NCRs match this filter.</p> : <div className="space-y-3">{visibleNcrs.map((item) => <div key={item.id} className="rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-medium">{item.title}</h3><p className="text-sm text-muted-foreground">Project #{item.projectId} · Assigned to {item.assignedTo || '—'}</p></div><div className="flex items-center gap-2"><Badge variant={item.severity === 'critical' ? 'destructive' : 'secondary'}>{item.severity}</Badge>{nextNcrStatus[item.status] && <Button variant="outline" size="sm" disabled={transitioning === item.id} onClick={() => void transition('ncr', item.id, nextNcrStatus[item.status] ?? '')}>تغییر به {nextNcrStatus[item.status]}</Button>}<Button variant="ghost" size="icon" aria-label="حذف گزارش عدم انطباق" onClick={() => void remove(`/non-conformance-reports/${item.id}`)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div></div><p className="mt-2 text-sm">{item.description}</p>{item.correctiveAction && <p className="mt-1 text-sm text-muted-foreground">Corrective action: {item.correctiveAction}</p>}<div className="mt-2 flex gap-2 text-xs text-muted-foreground"><Badge variant={statusTone(item.status)}>{item.status.replace('_', ' ')}</Badge>{item.dueDate && <span>Due {formatJalali(item.dueDate)}</span>}</div></div>)}</div>}</CardContent></Card>
      </div>
    </div>
  );
}
