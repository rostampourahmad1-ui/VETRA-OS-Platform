...
const FIELD_TYPE_KEYS: Record<FieldType, TranslationKey> = {
  text: 'forms.fieldText',
  number: 'forms.fieldNumber',
  date: 'forms.fieldDate',
  select: 'forms.fieldSelect',
  checkbox: 'forms.fieldCheckbox',
};

const STATUS_KEYS: Record<string, TranslationKey> = {
  draft: 'forms.statusDraft',
  published: 'forms.statusPublished',
  archived: 'forms.statusArchived',
  submitted: 'forms.submissionStatusSubmitted',
  approved: 'forms.submissionStatusApproved',
  rejected: 'forms.submissionStatusRejected',
};
...
const fieldTypes: Array<{ type: FieldType; label: string; icon: typeof Type }> = [
  ...
  { type: 'checkbox', label: t('forms.fieldCheckbox'), icon: ToggleLeft },
];
...
function createField(type: FieldType): FormField {
  ...
  return {
    ...
    type,
    required: false,
    placeholder: type === 'text' ? t('forms.responsePlaceholder') : undefined,
    options: type === 'select' ? ['گزینهٔ ۱', 'گزینهٔ ۲'] : undefined,
  };
}
...
export default function FormsBuilder() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((cause) => {
        if (active) setError(cause instanceof Error ? cause.message : t('forms.loadFailed'));
      })
      .finally(() => {
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t('forms.saveFailed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const publishDraft = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t('forms.publishFailed'));
    } finally {
      setSaving(false);
    }
  };
        ...
        <CardContent className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <div className="space-y-3">
            {submissions.length === 0 ? <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">{t('forms.noSubmissions')}</div> : submissions.map((submission) => {
              ...
              return <div key={submission.id} className="rounded-lg border p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-medium">{template?.name ?? t('forms.submissionPrefix', { id: submission.id })}</p><p dir="ltr" className="text-xs text-muted-foreground">#{submission.id} · {statusLabel(submission.status)}</p></div><Badge variant={submission.status === 'approved' ? 'default' : 'secondary'}>{statusLabel(submission.status)}</Badge></div>{pending && <div className="mt-3 flex flex-wrap gap-2"><Button size="sm" disabled={submissionBusy === submission.id} onClick={() => void decideSubmission(submission, 'approve')} className="gap-1"><CheckCircle2 className="h-3.5 w-3.5" /> {t('forms.approve')}</Button><Button size="sm" variant="outline" disabled={submissionBusy === submission.id} onClick={() => void decideSubmission(submission, 'request_revision')} className="gap-1"><RotateCcw className="h-3.5 w-3.5" /> {t('forms.requestRevision')}</Button><Button size="sm" variant="destructive" disabled={submissionBusy === submission.id} onClick={() => void decideSubmission(submission, 'reject')} className="gap-1"><XCircle className="h-3.5 w-3.5" /> {t('forms.reject')}</Button></div>}</div>;
            })}
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 xl:grid-cols-[240px_minmax(0,1fr)_300px]">
        {mode === 'build' && (
          <Card className="h-fit border-border/70 bg-card/80">
            <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-sm"><Plus className="h-4 w-4 text-primary" /> {t('forms.fieldPalette')}</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              {fieldTypes.map(({ type, label, icon: Icon }) => (
                <button key={type} type="button" onClick={() => addField(type)} className="flex w-full items-center gap-3 rounded-lg border border-transparent px-3 py-2.5 text-start text-sm transition-colors hover:border-primary/30 hover:bg-primary/5">
                  <span className="flex h-8 w-8 items-center justify-center rounded-md bg-muted text-primary"><Icon className="h-4 w-4" /></span>
                  <span>{label}</span>
                  <Plus className="ms-auto h-4 w-4 text-muted-foreground" />
                </button>
              ))}
              <div className="mt-4 rounded-lg bg-muted/60 p-3 text-xs leading-5 text-muted-foreground">{t('forms.paletteTip')}</div>
            </CardContent>
          </Card>
        )}
          ...
          <CardHeader className="border-b border-border/70 pb-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 space-y-2">
                <Input value={draft.name} onChange={(event) => updateDraft({ name: event.target.value })} className="h-9 max-w-lg border-0 bg-transparent px-0 text-xl font-semibold shadow-none focus-visible:ring-0" aria-label={t('forms.formName')} />
                <Textarea value={draft.description} onChange={(event) => updateDraft({ description: event.target.value })} className="min-h-12 max-w-xl resize-none border-0 bg-transparent px-0 text-sm shadow-none focus-visible:ring-0" aria-label={t('forms.formDescription')} />
              </div>
              <Badge variant="secondary" className="shrink-0 gap-1"><LayoutTemplate className="h-3 w-3" /> {statusLabel(draft.status)}</Badge>
            </div>
          </CardHeader>
          <CardContent className="space-y-4 p-5">
            {mode === 'preview' ? (
              <div className="mx-auto max-w-xl space-y-5 rounded-xl border bg-background p-6 shadow-sm">
                <div><h2 className="text-xl font-semibold">{draft.name || t('forms.untitledForm')}</h2><p className="mt-1 text-sm text-muted-foreground">{draft.description}</p></div>
            ...
            {!selectedField ? <p className="text-sm text-muted-foreground">{t('forms.selectFieldToEdit')}</p> : <>
              ...
              {(selectedField.type === 'number' || selectedField.type === 'text') && (
                <div className="space-y-3 pt-2 border-t">
                  <Label className="text-xs font-semibold text-muted-foreground">{t('forms.validation')}</Label>
                  {selectedField.type === 'number' && (
                    <>
                      <div className="space-y-2">
                        <Label htmlFor="field-min">{t('forms.minimum')}</Label>
                        <Input
                          id="field-min"
                          type="number"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="field-max">{t('forms.maximum')}</Label>
                        <Input
                          id="field-max"
                          type="number"
                  ...
                  {selectedField.type === 'text' && (
                    <div className="space-y-2">
                      <Label htmlFor="field-regex">{t('forms.regexPattern')}</Label>
                      <Input
                        id="field-regex"
                        dir="ltr"
                      ...
                      />
                      <p className="text-xs text-muted-foreground">{t('forms.customValidation')}</p>
                    </div>
                  )}
                </div>
              )}
...
function SortableFieldItem({ field, index, isSelected, onSelect, onRemove }: {
  ...
  return (
    <button
      ...
      type="button"
      onClick={onSelect}
      className={`group flex w-full items-center gap-3 rounded-xl border p-4 text-start transition-all ${
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="text-xs text-muted-foreground">
          {t('forms.responseType', { type: t(FIELD_TYPE_KEYS[field.type]) })} {index + 1}
        </span>
      </span>
      <Trash2
        onClick={(event) => {
...
function PreviewField({ field }: { field: FormField }) {
  const label = <Label>{field.label}{field.required && <span className="me-1 text-primary">*</span>}</Label>;
