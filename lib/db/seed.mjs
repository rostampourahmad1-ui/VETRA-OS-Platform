 ...
 *
 * Optional: set SEED_CLERK_USER_ID to map a real Clerk user into the demo
 * organization (as the admin role) so the signed-in user can see/edit the data.
 *
...
async function main() {
  ...
  console.log(`  clerk user provisioned: ${provisioned ? "yes" : "no (set SEED_CLERK_USER_ID to map your Clerk user)"}`);
